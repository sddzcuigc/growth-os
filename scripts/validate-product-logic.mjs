...
for (const term of ["draft: draft || undefined", "只追问草稿中尚未说清", "followup: isFollowup", "AbortSignal.timeout(12000)"]) if (!serverSource.includes(term)) failures.push(`journal follow-up server missing ${term}`);
for (const term of ["根据我写的继续问", "journalDraft", "ask-journal-followup", "state.journalDraft = \"\""]) if (!appSourceText.includes(term)) failures.push(`journal follow-up UI missing ${term}`);
